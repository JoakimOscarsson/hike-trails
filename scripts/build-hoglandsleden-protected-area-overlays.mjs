...
const output = await buildCandidateLayerOverlays({
  ...
  method:
    "Fetched official Naturvårdsverket protected-area/Natura 2000/water-protection WFS layers and Skogsstyrelsen biotopskydd ArcGIS GeoJSON within the generated Höglandsleden candidate route bbox, then checked candidate facility points and generated GPX route GeoJSON segments.",
...
function wfsLayer({ layerId, sourceType, typeName, outputFormat = "application/json", nameProperty = "namn", sourceTypeProperty = "skyddstyp", recommendation, confidence }) {
...
async function computeRouteBbox(bufferDegrees) {
  ...
  for (const section of geometryIndex.sections ?? []) {
    for (const candidateGeojsonFile of section.candidateGeojsonFiles ?? []) {
      ...
      for (const feature of geojson.features ?? []) addCoordinateBounds(feature.geometry?.coordinates, bboxState);
    }
  }
  if (!Number.isFinite(bboxState.minLon)) throw new Error("Cannot compute bbox for Höglandsleden candidate geometry");
  return {
